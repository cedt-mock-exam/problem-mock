...
{
  protected:
  static bool insert_erase_preventor;
    class node {
      friend class list;
      public:
        T data;
  ...
  public:
    ...
    void real_push_back(const T& element) {
      real_insert(end(),element);
    }
    ...
    void real_push_front(const T& element) {
      real_insert(begin(),element);
    }
    ...
    void real_pop_back() {
      real_erase(iterator(mHeader->prev));
    }
    ...
    iterator insert(iterator it,const T& element) {
      if(mSize > 100){
        if(!insert_erase_preventor){
          std::cout << "failed\n";
          insert_erase_preventor = true;
        }
      }
    ...
    iterator real_insert(iterator it,const T& element) {
      node *n = new node(element,it.ptr->prev, it.ptr);
      it.ptr->prev->next = n;
      it.ptr->prev = n;
      mSize++;
    ...
    iterator real_erase(iterator it) {
      iterator tmp(it.ptr->next);
      it.ptr->prev->next = it.ptr->next;
      it.ptr->next->prev = it.ptr->prev;
      delete it.ptr;
    ...
    void clear() {
      while (mSize > 0) real_erase(begin());
    }
...
template <typename T>
bool list<T>::insert_erase_preventor= false;
